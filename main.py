...
async def send_admin(context, text):
    ...
    try:
        ...
        logging.info("Admin message sent")
...
"""
        ...
        await update.message.reply_text(
            "🌸 سن کودک را وارد کنید:"
        )

        return CHILD_AGE


    elif text == "👴 مراقبت سالمند":

        await update.message.reply_text(
            "🌿 سن سالمند را وارد کنید:"
        )
    ...
    elif text == "🤝 معرفی نیرو":

        await update.message.reply_text(
            "آیا معرف دارید؟ (بله یا خیر)"
        )
